...
const missingGamesPath = path.join(__dirname, 'public', 'megaways_missing_with_iframes.json');
const missingGames = JSON.parse(fs.readFileSync(missingGamesPath, 'utf8'));

let finalGamesStr = `\n// --- MEGAWAYS GAMES EXTRACTED FROM SLOTRA ---\n`;
...
if (endOfDemoGames !== -1) {
    lobbyCode = lobbyCode.substring(0, endOfDemoGames) + finalGamesStr + '// --- END MEGAWAYS ---\n' + lobbyCode.substring(endOfDemoGames);
}

fs.writeFileSync(lobbyPath, lobbyCode, 'utf8');
console.log(`CasinoLobby.tsx successfully updated with ${missingGames.length} new Megaways games.`);
