...
# 1. Replace the HEADER block ONLY
old_header_pattern = r'\{\/\* HEADER \*\/\}\s*<div className="flex items-center justify-between px-5 sm:px-8 py-5 border-b border-white/5 shrink-0 bg-\[\#0F1423\] relative z-20">.*?<\/button>\s*<\/div>\s*<\/div>'
...
# 2. Delete the TABS block from the content area safely
tabs_pattern = r'\{\/\* TABS \*\/\}\s*<div className="flex bg-\[\#161B29\].*?<\/div>'
content = re.sub(tabs_pattern, '', content, flags=re.DOTALL)

# 3. Adjust paddings and height
