const fs = require('fs');
const path = '/Users/alex/Desktop/7_24bets-landing-page/components/sports/SportsPromoSlider.tsx';
let code = fs.readFileSync(path, 'utf8');

const newComponent = `export default function SportsPromoSlider({ matches = [], compact = false, onSelectMatch }: { matches?: any[], compact?: boolean, onSelectMatch?: (match: any) => void }) {
    ...
    const allSlides = useMemo(() => {
        ...
        // 1. Try to find API premium matches
        let availableMatches = soccerMatches.filter(m => 
            isPremium(m.league) && 
        ...
        // Fallback 1: Any match with logos
        if (availableMatches.length === 0) {
            availableMatches = soccerMatches.filter(m => findBestLogoMatch(m.home) !== null && findBestLogoMatch(m.away) !== null);
        }

        // Fallback 2: Any match at all
        if (availableMatches.length === 0) {
            availableMatches = soccerMatches.slice(0, 6);
        }

        availableMatches = availableMatches.slice(0, 8); // Max 8 matches
...
const exportIndex = code.indexOf('export default function SportsPromoSlider');
if (exportIndex !== -1) {
    code = code.substring(0, exportIndex) + newComponent;
