...
def scrape_bonusbuy_list():
    url = "https://slotra.com/casino/bonus-buy"
    print(f"{url} adresindeki Bonus Satın Alma oyunlarının LİSTESİ kazınıyor...")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            while True:
                ...
                if len(games_dict) >= 100:
                    print(f"İstenilen limit sayısına ({len(games_dict)}) ulaşıldı, kaydırma durduruluyor.")
                    break
            ...
            output_path = os.path.join("public", "bonusbuy_raw.json")
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(games_data, f, ensure_ascii=False, indent=2)
                
            print(f"Bonus Satın Al oyunları listesi '{output_path}' dosyasına kaydedildi.")
