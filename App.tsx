...
const BlackjackGame = React.lazy(() => import('./components/BlackjackGame'));
const BlackjackProView = React.lazy(() => import('./components/BlackjackProView'));
...
// Import removed
const PromoWheel = React.lazy(() => import('./components/PromoWheel'));
const WheelDashboard = React.lazy(() => import('./components/WheelDashboard'));
const LuckyWheelView = React.lazy(() => import('./components/LuckyWheelView'));
import { DEFAULT_GIVEAWAY_CONFIG } from './components/GiveawayView';
const GiveawayView = React.lazy(() => import('./components/GiveawayView'));
import SearchModal from './components/SearchModal';
import { DEFAULT_LOYALTY_CONFIG } from './components/LoyaltyPanel';
const LoyaltyPanel = React.lazy(() => import('./components/LoyaltyPanel'));
const RaffleView = React.lazy(() => import('./components/RaffleView'));
const RaffleLanding = React.lazy(() => import('./components/RaffleLanding'));
const CekilisCenterView = React.lazy(() => import('./components/CekilisCenterView'));
const PoolGame = React.lazy(() => import('./components/PoolGame'));
...
const TrustedSitesView = React.lazy(() => import('./components/TrustedSitesView'));
const TrustedDetailView = React.lazy(() => import('./components/TrustedDetailView'));
import { initTrustedEngine, loadTrustedCompanies, processDripComments, processAutoReplies } from './utils/trustedEngine';
const DemoGames = React.lazy(() => import('./components/DemoGames'));
import MyBetsModal from './components/MyBetsModal';
const KralView = React.lazy(() => import('./components/KralView'));
const WorldCupTeaser = React.lazy(() => import('./components/WorldCupTeaser'));
...
const PlinkoView = React.lazy(() => import('./components/PlinkoView'));
const CrashGameView = React.lazy(() => import('./components/CrashGameView'));
const LimboView = React.lazy(() => import('./components/LimboView'));
import OriginalsBottomBar from './components/OriginalsBottomBar';
const ChickenRunView = React.lazy(() => import('./components/ChickenRunView'));
const DiceView = React.lazy(() => import('./components/DiceView'));
const MinesView = React.lazy(() => import('./components/MinesView'));
const KenoView = React.lazy(() => import('./components/KenoView'));
const WarView = React.lazy(() => import('./components/WarView'));
const HiLoView = React.lazy(() => import('./components/HiLoView'));
const RewardsPage = React.lazy(() => import('./components/RewardsPage'));
const RouletteView = React.lazy(() => import('./components/RouletteView'));
const CrashTurboView = React.lazy(() => import('./components/CrashTurboView'));
const TurboMinesView = React.lazy(() => import('./components/TurboMinesView'));
const HacksawSlotView = React.lazy(() => import('./components/HacksawSlotView'));
...
const AppContent: React.FC<{ setIsAdminPanelOpen: (val: boolean) => void, initialView: any }> = ({ setIsAdminPanelOpen, initialView }) => {
  ...
  useEffect(() => {
    ...
    const handleNavigateTahmin = (e: CustomEvent<{ id: string }>) => {
      setView('tahmin-detay');
      window.history.pushState(null, '', `/tahmin/${e.detail.id}`);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    };
    ...
    window.addEventListener('navigate-tahmin', handleNavigateTahmin as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener('navigate-tahmin', handleNavigateTahmin as EventListener);
    ...
    <UserProvider siteUser={siteUser} setSiteUser={setSiteUser}>
      ...
      <div 
        ...
        className={`site-main-content flex-1 flex flex-col ${view === 'admin' ? 'admin-layout' : ''} ${
          (view === 'gercek' || view === 'sports' || view === 'sports2' || view === 'sports3' || view === 'sports4' || view === 'sports5' || view === 'spor724' || view === 'tahminler' || view === 'upcomingMatches' || view === 'limbo' || view === 'chicken-run' || view === 'originals' || view === 'blackjack' || view === 'slots' || view === 'live-casino' || view === 'favorites' || view === '724tv' || view === 'luckywheel' || view === 'raffle' || view === 'cekilis' || view === 'bulten' || ['blackjack-pro', 'limbo', 'chicken-run', 'plinko', 'dice', 'mines', 'keno', 'war', 'hilo', 'roulette', 'crash-turbo', 'turbo-mines', 'hacksaw', 'redtiger', 'crash'].includes(view)) 
      ...
      >
        ...
        {view === 'tahmin-detay' && (
          <div className="w-full h-full animate-fade-in">
            <MarketDetailView onNavigate={handleViewChange} />
          </div>
        )}
