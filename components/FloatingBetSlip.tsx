...
export const FloatingBetSlip = () => {
  ...
  return createPortal(
    <div className="floating-bet-slip fixed bottom-0 z-[60] flex flex-col items-end shadow-2xl font-sans w-full md:w-[350px] right-0 transition-all duration-300 border-l border-[#111111]">
      ...
      {/* EXPANDED CONTENT */}
      {isOpen && (
        <div className="w-full h-[calc(100vh-70px)] bg-[#0A0C10]/95 backdrop-blur-xl flex flex-col shadow-[-10px_0_40px_rgba(0,0,0,0.5)]">
          {/* TABS */}
          ...
          {betTab === 'Kombine' && bets.length >= 3 && (
            <div className="px-3 py-4 bg-[#0A0C10] border-b border-[#111111]">
              <div className="border border-[#10b981]/20 rounded-lg p-4 relative bg-[#0F121A] overflow-hidden">
                <div className="absolute top-0 right-0 flex -mt-0 mr-0 text-[9px] font-bold rounded-bl-lg overflow-hidden">
                  <span className="bg-[#0A0C10] text-white px-2 py-1">MİN ORAN: 1.50</span>
                ...
                <div className="flex gap-1 h-1.5 mt-2">
                  {[...Array(10)].map((_, i) => (
                    <div key={i} className={`flex-1 rounded-full transition-colors ${
                      i < validComboBets ? 'bg-[#10b981] shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-[#0A0C10]'
                    }`}></div>
                  ))}
                </div>
              </div>
          ...
          {/* STAKE INPUT */}
          <div className="px-4 py-5 bg-[#0A0C10] border-t border-[#111111]">
                ...
                <button 
                  ...
                  className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${
                    amount === val.toString() 
                      ...
                      : 'bg-[#0A0C10] border border-[#2A2E3D] text-[#848B9D] hover:text-[#10b981] hover:border-[#10b981]/30'
                  }`}
                >
                  {val}
                </button>
