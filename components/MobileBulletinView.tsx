...
const MobileBulletinView: React.FC<MobileBulletinViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-[#0A0C10]">
      <div className="flex items-center justify-between p-4 border-b border-white/5 bg-[#12151C]">
        <h2 className="text-xl font-bold text-white tracking-wide">
          Mobil <span className="text-[color:var(--theme-accent)]">Bülten</span>
        </h2>
        <button
          onClick={onBack}
      ...
      <div className="relative flex-1 w-full h-[calc(100vh-73px)] min-h-[800px]">
        {isLoading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0A0C10] z-10">
            <Loader className="w-8 h-8 text-[color:var(--theme-accent)] animate-spin mb-4" />
