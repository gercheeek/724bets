    ...
    return (
        <div className="flex flex-col md:flex-row w-full h-full bg-[#10171E] text-white font-sans overflow-y-auto md:overflow-hidden">
            ...
            <div className="w-full md:w-[320px] bg-[#222E3A] border-r border-[#151D24] p-4 flex flex-col shrink-0 z-20 shadow-2xl order-2 md:order-1 h-auto md:h-full overflow-y-auto">
                ...
                <div className="mb-6">
                    <div className="flex justify-between items-end mb-2">
                        <label className="text-xs text-gray-400 font-semibold">Bakiye</label>
                        <span className="text-xs text-[#ffd700] font-mono font-bold">${siteUser ? siteUser.balance.toFixed(2) : '0.00'}</span>
                    </div>
                    {totalBet > 0 && (
                        <div className="bg-[#151D24] rounded-md border border-[#2A3744] px-3 py-2 flex items-center mt-2">
                            <span className="text-gray-400 mr-2 text-xs font-bold uppercase">Total Bet:</span>
                            <span className="text-white font-mono text-sm">${totalBet.toFixed(2)}</span>
                        </div>
                    )}
                </div>
                ...
                {/* Chip Selection */}
                <div className="mb-6">
                    <label className="block text-xs text-gray-400 font-semibold mb-2">Çip Seç & Masaya Tıkla</label>
                    <div className="flex items-center gap-2">
                        {[10, 50, 100].map(val => (
                            <div key={val} className="transform scale-75 origin-left">
                                <CasinoChip 
                                    value={val} 
                ...
                <div className="flex flex-col gap-2 mt-auto">
                    {gameState === 'betting' && totalBet > 0 && (
                        <button onClick={handleClearBets} className="w-full bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold py-2 rounded-md transition-colors border border-red-500/20 mb-2">
                            Bahisleri Temizle
                        </button>
                    )}
                    ...
                    {gameState === 'betting' || gameState === 'ended' ? (
                        <button 
                            ...
                            className={`w-full font-black py-4 rounded-md transition-all shadow-lg uppercase tracking-widest text-lg ${
                                (totalBet > 0 || gameState === 'ended')
                                ? 'bg-emerald-500 hover:bg-emerald-600 text-white' 
                                : 'bg-[#324555] text-gray-400 cursor-not-allowed'
                            }`}
                        >
                            {gameState === 'ended' ? 'YENİ OYUN' : 'DAĞIT (DEAL)'}
                        </button>
                    ) : (
                        <div className="grid grid-cols-2 gap-2">
                            <button 
                                ...
                                onClick={handleHit}
                                className={`flex flex-col items-center justify-center py-3 rounded-md transition-all shadow-lg border-2 ${
                                    gameState === 'playing' 
                                    ? 'bg-gradient-to-b from-emerald-500 to-emerald-700 border-emerald-300 text-white hover:brightness-110' 
                                    : 'bg-[#324555] border-transparent text-gray-400 cursor-not-allowed'
                                }`}
                            >
                                <span className="text-xl">✋</span>
                                <span className="text-[10px] font-black uppercase tracking-wider mt-1">Hit (Al)</span>
                            </button>
                            ...
                            <button 
                                ...
                                className={`flex flex-col items-center justify-center py-3 rounded-md transition-all shadow-lg border-2 ${
                                    gameState === 'playing' 
                                    ? 'bg-gradient-to-b from-rose-500 to-rose-700 border-rose-300 text-white hover:brightness-110' 
                                    : 'bg-[#324555] border-transparent text-gray-400 cursor-not-allowed'
                                }`}
                            >
                                <span className="text-xl">🛑</span>
                                <span className="text-[10px] font-black uppercase tracking-wider mt-1">Stand (Dur)</span>
                            </button>
                        </div>
                    )}
                </div>
            ...
            <div className="flex-1 bg-[#10171E] relative overflow-hidden flex items-center justify-center p-2 sm:p-4 md:p-12 order-1 md:order-2 min-h-[400px] md:min-h-0">
                ...
                <div className="w-full max-w-5xl h-full max-h-[700px] relative rounded-3xl shadow-2xl overflow-hidden flex flex-col justify-between border-[6px] border-[#1C252D] bg-[#0A3D24]">
                    ...
                    <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg width=\'10\' height=\'10\' viewBox=\'0 0 10 10\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M0 0h1v1H0z\' fill=\'%23000\' fill-opacity=\'1\' fill-rule=\'evenodd\'/%3E%3C/svg%3E")' }}></div>

                    {/* Table Markings */}
                    <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[80%] h-[60%] border border-[#ffd700]/30 rounded-full pointer-events-none flex items-center justify-center">
                        <div className="text-[#ffd700]/20 font-black text-2xl tracking-[1em] uppercase whitespace-nowrap">
                    ...
                    <div className="w-full flex justify-center gap-4 md:gap-12 relative px-2 mb-12 z-10">
                        {hands.map((hand, i) => (
                    ...
                    {gameState === 'ended' && totalPayout > totalBet && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[4px] animate-fade-in pointer-events-none">
