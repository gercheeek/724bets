...
import { Target, TrendingUp, Activity, RefreshCw } from 'lucide-react';

export default function AdminPlinkoTab() {
  ...
  const loadData = () => {
    try {
      const data = JSON.parse(localStorage.getItem('site_plinko_bets') || '[]');
      // Sort newest first
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
            <Target className="w-6 h-6 text-[#27D26D]" />
            Plinko Yönetimi (PRO)
          </h2>
          <p className="text-gray-400 text-sm mt-1">Gerçek zamanlı bakiye ile oynanan oyunların istatistikleri ve logları.</p>
        </div>
        <button 
          onClick={loadData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-[#151821] p-6 rounded-xl border border-white/5 relative overflow-hidden">
          ...
          <p className="text-gray-400 text-sm font-semibold mb-2">Toplam Oyun (Bet)</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#111317] border-b border-white/5 text-xs text-gray-400">
                ...
                <th className="py-3 px-4 font-semibold">Çarpan</th>
                <th className="py-3 px-4 font-semibold">Kazanç</th>
              ...
              {logs.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-gray-500 italic">
                    Henüz oynanan bir Plinko oyunu yok.
                  </td>
                </tr>
              ) : (
                logs.map((log) => (
                  <tr key={log.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        log.multiplier > 1 ? 'bg-[#27D26D]/20 text-[#27D26D]' : 'bg-red-500/20 text-red-400'
                      }`}>
                        {log.multiplier}x
                      </span>
                    </td>
