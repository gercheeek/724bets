      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-2">
            <Target className="text-[#06b6d4]" /> Spor Bülteni Yönetimi
          </h2>
          <p className="text-zinc-400 mt-1">Sitedeki bahis oranlarını, canlı skorları ve maçları buradan anlık yönetin.</p>
        </div>
        <button 
          onClick={() => setShowForm(!showForm)}
          className="bg-[#06b6d4] hover:bg-[#00E676] text-black font-black px-4 py-2 rounded-xl flex items-center gap-2 transition-colors"
        >
          ...
          <form onSubmit={handleAddMatch} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select 
                ...
                className="bg-[#0A0C10] border border-[#202532] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#06b6d4]"
              >
            ...
            <div className="md:col-span-2 lg:col-span-5 flex justify-end mt-2">
              <button type="submit" className="bg-[#06b6d4] text-black font-bold px-6 py-2 rounded-lg flex items-center gap-2">
              ...
              <div className="flex items-center gap-4">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-2 text-lg font-black">
                    ...
                    <span className="text-[#06b6d4] bg-[#0A0C10] px-2 py-0.5 rounded border border-[#202532]">{match.score_home} - {match.score_away}</span>
              ...
              <div className="lg:col-span-3 bg-[#0A0C10] rounded-lg p-3 border border-[#202532] flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <input type="number" value={match.score_home} onChange={e => updateMatch(match.id, { score_home: Number(e.target.value) })} className="w-full bg-[#12161E] border border-[#202532] rounded px-2 py-1 text-sm font-bold text-center focus:border-[#06b6d4] outline-none" />
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    <label className="text-[10px] text-zinc-400">Dep</label>
                    <input type="number" value={match.score_away} onChange={e => updateMatch(match.id, { score_away: Number(e.target.value) })} className="w-full bg-[#12161E] border border-[#202532] rounded px-2 py-1 text-sm font-bold text-center focus:border-[#06b6d4] outline-none" />
                  </div>
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-[10px] text-zinc-400">Dakika / Durum (Örn: 45', Devre Arası)</label>
                  <input type="text" value={match.match_minute} onChange={e => updateMatch(match.id, { match_minute: e.target.value })} placeholder="Örn: 45', Devre Arası" className="w-full bg-[#12161E] border border-[#202532] rounded px-2 py-1 text-sm font-bold focus:border-[#06b6d4] outline-none" />
                </div>
              </div>
                    ...
                    {[
                      ...
                      <div key={odd.key} className="flex flex-col gap-1 bg-[#12161E] p-2 rounded border border-[#202532]">
                        ...
                        <input 
                          ...
                          className="w-full bg-transparent border-none text-center text-[#06b6d4] font-black outline-none"
                        />
                      </div>
                    ))}
                 </div>
