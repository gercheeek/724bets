...
const slides = [
  {
    ...
    title: <><span className="text-transparent bg-clip-text bg-gradient-to-r from-[color:var(--theme-accent)] to-[#00B273]">iPhone 17 Pro Max</span> Çekilişi</>,
        ...
        {slides.map((slide, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
              ...
              <div className="absolute top-0 left-0 w-1/2 h-full bg-[color:var(--theme-accent)]/5 blur-[100px] pointer-events-none"></div>
            </div>
        ...
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
          {slides.map((_, idx) => (
            <button 
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-6 bg-[color:var(--theme-accent)]' : 'w-2 bg-white/20 hover:bg-white/40'}`}
            />
          ))}
        </div>
      </div>
