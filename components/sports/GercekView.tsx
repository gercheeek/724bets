...
export const GercekView: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-full bg-transparent text-slate-100 p-4 md:p-6 lg:px-8 selection:bg-blue-600 selection:text-white">
      ...
      {/* ── TOP HEADER SECTION ── */}
      <div id="canli-maclar" className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 scroll-mt-6">
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-white/10 shrink-0">
            <span className="w-2.5 h-2.5 rounded-full bg-white relative z-10 animate-pulse"></span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-white tracking-wide flex items-center gap-2">
            Canlı
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/30">
      ...
      <div className="flex items-center gap-4 overflow-x-auto pb-3 mb-6 scrollbar-none border-b border-white/5">
        <div
          onClick={() => setSelectedCategory('all')}
          className="relative group cursor-pointer transition-all duration-200 shrink-0"
        >
          {/* Custom Skewed Background */}
          <div className={`absolute inset-0 -skew-x-12 transition-all duration-300 ${
            selectedCategory === 'all'
              ? 'bg-[#1a1a1a] border-b-2 border-[#00ff88] shadow-[0_5px_15px_-5px_rgba(0,255,136,0.3)]'
              : 'bg-transparent border-b-2 border-transparent group-hover:bg-white/5'
          }`}></div>
          
          <div className="relative z-10 px-5 py-2 md:py-2.5 flex items-center gap-2">
            <Flame className={`w-4 h-4 md:w-5 md:h-5 stroke-[2.5] transition-colors ${
              selectedCategory === 'all' ? 'text-[#00ff88]' : 'text-zinc-500 group-hover:text-zinc-300'
            }`} />
            <span className={`font-black text-xs md:text-sm tracking-wide uppercase transition-colors ${
              selectedCategory === 'all' ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'
            }`}>
              TÜMÜ
            </span>
            <span className={`text-[10px] md:text-xs font-bold px-2 py-0.5 ml-1 transition-colors ${
              selectedCategory === 'all' 
                ? 'bg-[#00ff88] text-black skew-x-12' 
                : 'bg-zinc-800 text-zinc-400 group-hover:bg-zinc-700 skew-x-12'
            }`}>
              <div className="-skew-x-12">{matches.length}</div>
            </span>
          </div>
        </div>
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10 px-5 py-2 md:py-2.5 flex items-center gap-2">
                <div className={isActive ? 'text-[#00ff88]' : 'text-zinc-500 group-hover:text-zinc-300'}>
                  {cat.icon}
                </div>
                <span className={`font-black text-xs md:text-sm tracking-wide uppercase transition-colors ${
                  isActive ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'
                ...
                <span className={`text-[10px] md:text-xs font-bold px-2 py-0.5 ml-1 transition-colors ${
                  ...
                  <div className="-skew-x-12">{cat.count}</div>
                </span>
              </div>
            </div>
          );
            ...
            return (
              <div className="flex flex-col gap-6 mt-4">
                ...
                {sortedLeagues.map(([leagueName, leagueMatches]) => (
                  <div key={leagueName} className="bg-transparent border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 hover:border-white/10 mb-4 opacity-90 hover:opacity-100">
                    {/* League Header */}
                    <div className="backdrop-blur-xl bg-white/[0.03] px-4 py-3 flex items-center gap-3 relative border-b border-white/5">
                      <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full">
                        <div className="w-1.5 h-1.5 bg-white/80 rounded-full" />
                        ...
                        <div key={match.id} className={`flex flex-col md:flex-row md:items-center gap-4 p-5 hover:bg-white/[0.03] transition-all duration-300 relative group overflow-hidden ${index !== 0 ? 'border-t border-white/[0.03]' : ''}`}>
                          ...
                          <div className="flex-1 flex flex-col gap-3 min-w-0">
                            <div className="flex items-center justify-between gap-3 pr-2 md:pr-8">
                              ...
                              <span className="text-transparent bg-clip-text bg-gradient-to-b from-white/50 to-white/10 font-black text-[11px] uppercase shrink-0 tracking-widest mt-0.5 italic drop-shadow-sm px-2">vs</span>
                            </div>
                            <div className="flex items-center justify-between gap-3 pr-2 md:pr-8">
                              <div className="flex items-center gap-3 truncate">
                                <img src={`https://flagcdn.com/w80/${['it','fr','br','ar','pt','nl','be','gb','de','es'][match.team2.name.length % 10]}.png`} className="w-6 h-6 md:w-7 md:h-7 rounded-full shadow-sm shrink-0 border border-white/10 object-cover" alt={match.team2.name} />
