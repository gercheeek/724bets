...
export default function SportsPromoSlider({ matches = [] }: { matches?: any[] }) {
    ...
    const dynamicSlides = useMemo(() => {
        ...
        if (fenerMatch) {
            slides.push({
                data: {
                    home: fenerMatch.home, away: fenerMatch.away,
                    homeOdd: fenerMatch.homeOdd, drawOdd: fenerMatch.drawOdd, awayOdd: fenerMatch.awayOdd,
                    dateStr: fenerMatch.matchDate || 'BUGÜN', timeStr: fenerMatch.startTime || '20:00'
                },
                theme: 'fener', name: 'SÜPER LİG ÖZEL', id: fenerMatch.id
            });
        }
        ...
        const confMatch = matches.find((m: any) => {
            const l = (m.league || '').toUpperCase();
            return (l.includes('CONFERENCE') || l.includes('KONFERANS')) && m.id !== fenerMatch?.id && m.id !== clMatch?.id && m.id !== elMatch?.id && hasLogos(m);
        });
        if (confMatch) {
            slides.push({
                data: {
