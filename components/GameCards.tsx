...
export const BaseGameCard: React.FC<{ game: any, onClick: () => void, variant?: 'cyan' | 'green' | 'purple' | 'gold' }> = ({ game, onClick, variant = 'cyan' }) => {
  const colors = {
    ...
    green: { border: 'hover:border-[#10B981]/50', shadow: 'hover:shadow-[0_10px_40px_rgba(16,185,129,0.25)]', glow: 'from-[#10B981]/20', ring: 'group-hover:ring-[#10B981]/50' },
    purple: { border: 'hover:border-[#A158FF]/50', shadow: 'hover:shadow-[0_10px_40px_rgba(161,88,255,0.25)]', glow: 'from-[#A158FF]/20', ring: 'group-hover:ring-[#A158FF]/50' },
    gold: { border: 'hover:border-[#F5A623]/50', shadow: 'hover:shadow-[0_10px_40px_rgba(245,166,35,0.25)]', glow: 'from-[#F5A623]/20', ring: 'group-hover:ring-[#F5A623]/50' }
  };
  const theme = colors[variant];

  return (
    <div 
      className={`group relative flex flex-col cursor-pointer rounded-2xl overflow-hidden transition-all duration-500 hover:z-10 border border-white/5 bg-[#0A0D14] ${theme.border} shadow-[0_4px_15px_rgba(0,0,0,0.5)] ${theme.shadow} hover:-translate-y-2 w-full`}
      tabIndex={0}
      onClick={onClick}
    >
...
export const NewGameCard: React.FC<{ game: any, onClick: () => void, onDemoClick?: () => void }> = (props) => <BaseGameCard {...props} variant="green" />;
