...
const getLeagueFlag = (league: string): string => {
    ...
    if (lowerLeague.includes('premier') || lowerLeague.includes('championship') || lowerLeague.includes('ingiltere')) return '🏴󠁧󠁢󠁥󠁮󠁧󠁿';
    if (lowerLeague.includes('la liga') || lowerLeague.includes('ispanya')) return '🇪🇸';
    if (lowerLeague.includes('serie a') || lowerLeague.includes('italya')) return '🇮🇹';
    if (lowerLeague.includes('bundesliga') || lowerLeague.includes('almanya')) return '🇩🇪';
    if (lowerLeague.includes('ligue 1') || lowerLeague.includes('fransa')) return '🇫🇷';
    if (lowerLeague.includes('süper lig') || lowerLeague.includes('türkiye') || lowerLeague.includes('1. lig')) return '🇹🇷';
    if (lowerLeague.includes('eredivisie') || lowerLeague.includes('hollanda')) return '🇳🇱';
    if (lowerLeague.includes('primeira') || lowerLeague.includes('portekiz')) return '🇵🇹';
    if (lowerLeague.includes('mls') || lowerLeague.includes('nba') || lowerLeague.includes('abd')) return '🇺🇸';
    if (lowerLeague.includes('brasileirao') || lowerLeague.includes('brezilya')) return '🇧🇷';
    if (lowerLeague.includes('şampiyonlar ligi') || lowerLeague.includes('avrupa ligi') || lowerLeague.includes('konferans') || lowerLeague.includes('euroleague') || lowerLeague.includes('uefa')) return '🇪🇺';
...
const AnalysisView: React.FC<AnalysisViewProps> = ({ analyses = [], coupons = [], siteUser = null, isLoggedIn = false, onLoginRequired }) => {
    ...
    const leagues = ['TÜMÜ', ...Array.from(new Set(combinedAnalyses.filter(a => {
        const matchesDate = selectedDate === 'WEEKLY' ? dates.includes(a.matchDate) : a.matchDate === selectedDate;
                ...
                <div className="w-full max-w-6xl mx-auto px-2 text-center relative">
                    <div className="flex overflow-x-auto justify-start md:justify-center gap-2 mb-4 pb-1 scrollbar-none px-2 relative">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'Futbol'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'Basketbol'
                                ? 'bg-[#FFC300] text-black shadow-[0_0_15px_rgba(255,195,0,0.4)] scale-105'
                                : 'bg-zinc-800/80 text-white hover:bg-zinc-700 border border-white/5'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'Formula 1'
                                ? 'bg-[#FFC300] text-black shadow-[0_0_15px_rgba(255,195,0,0.4)] scale-105'
                                : 'bg-zinc-800/80 text-white hover:bg-zinc-700 border border-white/5'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'MotoGP'
                                ? 'bg-[#FFC300] text-black shadow-[0_0_15px_rgba(255,195,0,0.4)] scale-105'
                                : 'bg-zinc-800/80 text-white hover:bg-zinc-700 border border-white/5'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'Superbike'
                                ? 'bg-[#FFC300] text-black shadow-[0_0_15px_rgba(255,195,0,0.4)] scale-105'
                                : 'bg-zinc-800/80 text-white hover:bg-zinc-700 border border-white/5'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-extrabold uppercase text-[10px] tracking-wider shrink-0 transition-all duration-300 backdrop-blur-md ${selectedSport === 'Tenis'
                                ? 'bg-[#FFC300] text-black shadow-[0_0_15px_rgba(255,195,0,0.4)] scale-105'
                                : 'bg-zinc-800/80 text-white hover:bg-zinc-700 border border-white/5'
                                }`}
                        >
                    ...
                    <div className="flex justify-center mb-4 relative">
                        <div className="flex overflow-x-auto gap-1.5 p-1 bg-[#1A1A1A]/80 backdrop-blur-xl border border-white/10 rounded-2xl scrollbar-none max-w-full">
                            ...
                            >
                                <span className="text-[9px] font-extrabold uppercase tracking-widest leading-none mb-0.5">TÜM</span>
                                <span className="text-[11px] font-extrabold leading-none">HAFTA</span>
                            </button>
                            ...
                            {dates.map((date, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="text-[9px] font-semibold tracking-wider mb-0.5 capitalize">{dayName}</span>
                                        <span className={`text-[11px] font-extrabold ${isSelected ? 'text-white' : 'text-zinc-400'}`}>{d.toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' })}</span>
                                        {isSelected && (
                    ...
                    <div className="flex overflow-x-auto gap-2 mb-2 pb-1 justify-start md:justify-center scrollbar-none px-2 relative">
                        {leagues.map((league) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider transition-all duration-300 shrink-0 ${selectedLeague === league
                                    ? 'bg-[#FFC300] text-black shadow-[0_0_10px_rgba(255,195,0,0.3)]'
                            ...
                            >
                                {league !== 'TÜMÜ' && <span className="mr-1">{getLeagueFlag(league)}</span>}
                                {league}
                            </button>
                        ))}
                    </div>
                        ...
                        <div key={leagueName} className="space-y-4">
                            ...
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    <h3 className="text-white font-extrabold text-[14px] md:text-[15px] tracking-wider leading-relaxed break-words flex items-center gap-2" title={leagueName}>
                                        <span className="text-lg">{getLeagueFlag(leagueName)}</span>
                                        {leagueName}
                                    </h3>
                                    <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mt-0.5 ml-2">({leagueMatches.length} YAYIN)</span>
                                </div>
                                <div>
