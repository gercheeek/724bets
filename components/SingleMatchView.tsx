  ...
  return (
    <div className="flex flex-col h-full bg-[#09090b] text-white animate-in fade-in slide-in-from-bottom-4 duration-300 relative z-30">
      ...
      <div className="flex items-center gap-4 px-6 py-4 border-b border-[#202532] bg-[#0E1116] sticky top-0 z-20">
        <button 
          onClick={onBack}
          className="w-8 h-8 flex items-center justify-center rounded bg-[#1A1D24] text-zinc-400 hover:text-[#10B981] hover:bg-[#10B981]/10 transition-colors border border-transparent hover:border-[#10B981]/30"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex flex-col">
          <span className="text-[#10B981] text-[10px] font-bold uppercase tracking-widest">{match.sport === 'futbol' ? 'Futbol / ' + (match.time === 'Yakında' ? 'Bülten' : 'Canlı') : match.sport}</span>
      ...
      <div className="relative overflow-hidden border-b border-[#202532]">
        <div className="absolute inset-0 bg-gradient-to-r from-[#10B981]/5 via-transparent to-[#10B981]/5"></div>
        ...
        <div className="relative px-8 py-10 flex items-center justify-between">
          ...
          <div className="flex flex-col items-center px-8">
            ...
            {match.isLive ? (
              <div className="flex items-center gap-4 text-5xl font-black text-[#10B981] tracking-tighter drop-shadow-[0_0_15px_rgba(0,255,163,0.3)]">
      ...
      <div className="flex-1 p-6 space-y-6 overflow-y-auto bg-[#09090b]">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <div className="text-[#10B981] font-bold text-sm tracking-widest animate-pulse">BAHİSLER YÜKLENİYOR...</div>
          </div>
        ) : (
                ...
                <div key={market.id} className="bg-[#12161E] rounded-xl border border-[#202532] overflow-hidden shadow-sm">
                  ...
                  <div className={`p-4 grid gap-2 ${market.selections.length > 3 ? 'grid-cols-3' : `grid-cols-${market.selections.length}`}`}>
                    {market.selections.map((opt: any, i: number) => (
                      <button key={i} className="group relative flex flex-col items-center justify-center h-14 bg-[#1A1D24] border border-[#202532] rounded hover:border-[#10B981]/50 hover:bg-[#10B981]/5 transition-all overflow-hidden">
                        <span className="text-zinc-500 text-[10px] font-bold group-hover:text-zinc-300">{opt.label}</span>
                        <span className="text-white font-black mt-0.5 group-hover:text-[#10B981]">{opt.odds}</span>
                      </button>
                    ))}
                  </div>
                </div>
