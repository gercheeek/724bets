...
const AuthModal: React.FC<AuthModalProps> = ({ mode, onMemberLogin, onAdminLogin, onClose, hideMemberLogin = false, initialMemberMode = 'login' }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[999999] flex items-center justify-center bg-black/80 backdrop-blur-[24px] ${showSplash ? 'p-0 md:p-4' : 'p-4'}`}>
            ...
            `}</style>
            <div className={`w-full max-w-md bg-[#111317] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] relative flex flex-col border border-white/5 ${showSplash ? 'h-[100dvh] rounded-none md:h-auto md:min-h-[500px] md:rounded-2xl' : 'h-auto min-h-[500px] max-h-[95vh] rounded-2xl md:rounded-2xl'}`}>
                
                {/* Auth Form Only */}
                <div className={`w-full p-4 sm:p-5 md:p-8 flex-col relative bg-gradient-to-b from-[#1a1d27] to-[#12141c] overflow-y-auto scrollbar-hide animate-in fade-in duration-700 flex ${showSplash ? 'hidden' : 'flex flex-1'}`}>
