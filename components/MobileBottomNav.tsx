      ...
      <div className="flex items-center justify-between px-1 sm:px-2 h-16 w-full max-w-full">
        {items.map((item) => {
          ...
          return (
            ...
            >
              <div className={`transition-all duration-300 ${isActive ? 'text-[#06b6d4] -translate-y-0.5' : 'text-gray-500'}`}>
                <Icon size={20} className={isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(0,255,163,0.5)]' : ''} />
              </div>
              <span className={`text-[9px] sm:text-[10px] font-bold tracking-wide transition-colors ${isActive ? 'text-[#06b6d4]' : 'text-gray-500'}`}>
                {item.label}
              </span>
            </button>
          );
