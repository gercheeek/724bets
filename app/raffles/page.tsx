...
import type { Raffle, ClaimCodeResult } from '@/types';
import CountdownTimer from './CountdownTimer';
import LiveCodeBox from './LiveCodeBox';
import HowToJoin from './HowToJoin';

// ── Mock data shapes matching your DB schema ──────────────────────
...
// ── ICONS ──────────────────────────────────────────────────────────
const NeonBolt = () => (
  <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5" stroke="#00E676" strokeWidth={2}>
    <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

// ── RAFFLE CARD (side) ─────────────────────────────────────────────
function SideRaffleCard({ raffle }: { raffle: RaffleWithMeta }) {
    ...
    `}>
      ...
      <div className="relative h-40 bg-zinc-900 flex items-center justify-center overflow-hidden">
        <img
          ...
          onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/280x160/111/333?text=' + raffle.title; }}
        />
        {isFull && (
...
function FeaturedRaffleCard({
  ...
  // Real-time subscription handled in parent, receives updates via prop
  return (
    <div className="relative rounded-3xl border border-[#00E676]/20 bg-gradient-to-br from-[#0d1a10] to-[#0d0d0d] overflow-hidden shadow-[0_0_60px_rgba(0,230,118,0.06)]">
      {/* Background glow */}
      <div className="absolute top-0 right-0 w-[380px] h-[380px] rounded-full bg-[#00E676]/4 blur-3xl pointer-events-none" />
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Update featured raffle ticket count
      ...
      .on(
        // When a live_code reaches max_winners → is_active becomes false
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'live_codes' },
        (payload) => {
          // Trigger a toast or badge update — handled inside LiveCodeBox
          console.log('[RT] live_code updated:', payload.new);
        }
      )
      .subscribe();
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-10 space-y-12">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {sideRaffles.map(r => (
              <SideRaffleCard key={r.id} raffle={r} />
            ))}
          </div>
        </section>
