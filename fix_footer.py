...
def fix_footer():
    with open('/Users/alex/Desktop/7_24bets-landing-page/components/Footer.tsx', 'r') as f:
        content = f.read()

    # I'll create a completely new, ultra-premium Gamdom style footer.
    # Gamdom footer uses #0b0e14 or #14171d background.
    # Has a lot of secure badges, crypto logos, very sleek typography.
    
    # We will replace the entire return block of the Footer.
    target_pattern = r'return \(.*?\);'
    ...
    content = re.sub(target_pattern, new_footer, content, flags=re.DOTALL)
    
    # We must ensure all imported icons are correct (Lock, CheckCircle2, AlertOctagon, Activity)
    import_target = r"import \{ (.*?) \} from 'lucide-react';"
    match = re.search(import_target, content)
    if match:
        icons_str = match.group(1)
        icons = set([i.strip() for i in icons_str.split(',')])
        needed = {'Shield', 'Lock', 'CheckCircle2', 'AlertOctagon', 'Activity'}
        icons.update(needed)
        new_import = f"import {{ {', '.join(sorted(icons))} }} from 'lucide-react';"
        content = content.replace(match.group(0), new_import)

    with open('/Users/alex/Desktop/7_24bets-landing-page/components/Footer.tsx', 'w') as f:
        f.write(content)
