...
def scrape_egt_search():
    url = "https://slotra.com/casino/search?q=egt"
    games_dict = {}
    ...
    with sync_playwright() as p:
        ...
        try:
            print(f"EGT araması başlatılıyor: {url}")
            ...
            while True:
                ...
                if len(games_dict) >= 50:
                    print("50 oyun limitine ulaşıldı.")
                    break
                ...
                if current_scroll >= new_height:
                    ...
                    if current_scroll >= final_height:
                        print("Arama sonuçlarının sonuna ulaşıldı.")
                        break
    ...
    output_path = os.path.join("public", "egt_search_raw.json")
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(games_data, f, ensure_ascii=False, indent=2)
        
    print(f"\nToplam {len(games_data)} oyun bulundu ve '{output_path}' dosyasına kaydedildi.")
