...
const startIndex = content.indexOf('const DUMMY_MATCHES: Match[] = [');
...
let generatedMatches = 'const DUMMY_MATCHES: Match[] = [\n';
for (let i = 0; i < 15; i++) {
...
// add one non live match to keep featured working maybe?
generatedMatches += `  {
    id: 'f_m1',
    sport: 'futbol',
    league: 'Gelecek Maçlar',
    minute: "Bugün 21:00",
    period: 'Maç Öncesi',
    hasStream: false,
    hasStats: true,
    team1: { name: 'Başakşehir', score: 0, logo: '⚽', color: '#ef4444' },
    team2: { name: 'Sivasspor', score: 0, logo: '⚽', color: '#3b82f6' },
    odds: { home: '2.10', draw: '3.20', away: '3.50' },
    totalMarkets: 10,
    isFavorite: true
  }\n];`;

content = content.slice(0, startIndex) + generatedMatches + content.slice(endIndex);
