...
content = content.replace(
    "} from 'lucide-react';",
    "Calendar, Tv, Dices, Radio } from 'lucide-react';"
);

// 2. We need to replace the entire `isOpen ? (...) : (...)` section.
// It starts at `{isOpen ? (` and ends before `</div>\n      </div>\n    </>`
// Let's find the exact string indices or use a precise regex, or better, just write the entire return block.
