...
# 1. Update InGameLayout condition
old_condition = "{(['blackjack-pro', 'limbo', 'chicken-run', 'plinko', 'dice', 'mines', 'keno', 'war', 'hilo', 'roulette', 'crash-turbo', 'turbo-mines', 'hacksaw', 'redtiger'].includes(view)) ? ("
new_condition = "{(['blackjack-pro', 'hacksaw', 'redtiger'].includes(view)) ? ("
content = content.replace(old_condition, new_condition)

# 2. Update InGameLayout gameTitle
old_title = """          gameTitle={
              view === 'limbo' ? 'Limbo' : 
...
# 3. Remove Original Games from InGameLayout children
games_to_remove = [
...
# 4. Insert Original Games into OriginalsHub area
originals_hub_block = """        {view === 'originals' && (
...
new_originals_hub_block = """        {/* ORIGINALS SECTION & GAMES */}
        {(() => {
...
content = content.replace(originals_hub_block, new_originals_hub_block)
