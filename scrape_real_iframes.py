...
def scrape_iframes():
    json_path = os.path.join("public", "slots_all_new.json")
    if not os.path.exists(json_path):
        print(f"HATA: {json_path} bulunamadı. Lütfen önce scrape_all_games.py scriptini çalıştırın.")
        return

    with open(json_path, "r", encoding="utf-8") as f:
        games = json.load(f)

    print(f"Toplam {len(games)} oyun yüklendi. İşlem başlıyor...")

    with sync_playwright() as p:
        # Görünür modda başlat, gerektiğinde manuel Cloudflare doğrulaması yapabilmek için
        ...
        # Cloudflare'i bir kez aşmak için ana sayfaya git
        ...
        for i, game in enumerate(games):
            original_link = game.get("iframe_url") or game.get("original_link")
            ...
            def handle_request(request):
                nonlocal real_url
                url = request.url
                # Sağlayıcı linklerini yakalamak için genel kelimeler
            ...
            try:
                ...
                # Linkin yakalanması için biraz bekle
                time.sleep(4)
                ...
                if real_url:
                    ...
                    # Alternatif olarak DOM'daki iframe src'sine bak
            ...
            time.sleep(2) # Ban yememek için bekle

        # Sonuçları yeni dosyaya kaydet
        output_path = os.path.join("public", "slots_all_new_with_iframes.json")
